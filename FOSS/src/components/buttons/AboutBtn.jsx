...
const AboutBtn = ({pageWidth}) => {
    ...
    }, [fromTop, topDistance, pageWidth]);

    const { display } = useSelector((store) => store.hide);
    const [isHovering, setIsHovering] = useState(false);

    let repositionBtn = {
        top: pageWidth > 768 ? '4.4rem' : '4.8rem',
        right: "var(--right)",
        opacity: pageWidth > 580 ? 1 : 0,
    };
    let hideStyle = {
        opacity: isHovering ? 1 : pageWidth > 480 ? "var(--semiOpaque)" : 0,
