...
import MouseDivImg from "../components/MouseDivImg";

const Home = ({ loading, pageWidth }) => {
  const dispatch = useDispatch();
  const { navOpen } = useSelector((store) => store.nav);
  const { showImg, inResume } = useSelector((store) => store.mouseImg);
  ...
  return (
    <>
      ...
      {showImg && <MouseDivImg posX={posX} posY={posY} inResume={inResume} />}
      <ScreenSweeper />
      {!loading && <Logo setToggle={setToggle} display={display} />}
      <AboutBtn pageWidth={pageWidth} />
      <WorkBtn />
      <Burger />
      {navOpen && <MobileNav />}
      <ContactBtn pageWidth={pageWidth} />
      <ResumeBtn />
      <SocialLinks />
      <Video />
      <Hero />
